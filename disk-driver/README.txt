...
opt to emulate this device and configure Bochs to use a simple file as a
virtual disk:

1) Create a virtual disk
  The first step is to create a virtual disk. The virtual disk will be a file
  ...
  '''''
  (This size should be more than enough for our tests).
...
2) Configure Bochs
  ...
  disk are:
    - the IO addresses to access the disk (0x1f0 and 0x3f0),
    - the interrupt number to use (14) and
    - a description of the disk (in this case the file 'fs.img' and the 'flat'
      access mode).
  Add the following lines to the _.bochsrc_ file:
  '''''
...
3) Use the driver
  After the previous steps, the driver may be used in your code. The first
  thing to do is to initialize the disk, and after that the disk may be read
  and written. Due to the current implementation of the driver, there is no
  way to get information about the disk, for example the number of blocks
  available in the disk and therefore this information needs to be "hardcoded"
  someway. For example, with the virtual disk created in the first step you
  'know' that the disk has a maximum of 10000 blocks.
  Any write operation using the driver will physically write the 'virtual'
...
<http://bochs.sourceforge.net/doc/docbook/user/bochsrc.html#BOCHSOPT-ATA>.
 ...
 vim:tw=78:ts=8:noet:ft=less:norl:
